...
public class TransparentBackgroundScreenshotRecorderSimple : MonoBehaviour {
    ...
    void LateUpdate()
    {
        ...
        {
            Debug.Log("Complete! " + totFrames + " videoframes rendered. File names are 0 indexed)");
            Debug.Break();
        }
    }
    ...
    {
        while (!GameController.instance.getIsReady())
        {
            yield return null;
        }
        ...
        {
            GameController.instance.SetBlendshapeValue((float)(100f * frameNum) / totFrames);
            CaptureFrame(frameNum);
            frameNum++;
        }
        else
        {
    ...
    public void CaptureFrame (int shotNum){
	...
	void SavePng (int shotNum){
		string name = string.Format ("{0}/{1:D04} shot.png", folderName, shotNum);
